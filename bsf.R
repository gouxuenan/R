...
layerB <- filter(df, layer == "B") %>%
  select(!layer & !ID & !type & !group) %>%
  corr.test(use = "complete", method = "pearson", adjust = "none")

corrplot(layerB$r, p.mat = layerB$p, insig ="label_sig",sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, pch.col = "white", type = "upper", tl.pos = "d", tl.col = "black")
...
typeLYS <- filter(df, type == "LYS") %>%
  select(!layer & !ID & !type & !group) %>%
  corr.test(use = "complete", method = "pearson", adjust = "none")
corrplot(typeLYS$r, p.mat = typeLYS$p, insig ="label_sig",sig.level = c(0.001, 0.01, 0.05), pch.cex = 1.5, pch.col = "white", type = "upper", tl.pos = "d", tl.col = "black")
